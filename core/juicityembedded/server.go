...
import (
	...
	"github.com/wyx2685/v2node/core/juicityembedded/relay"
...
func (s *Server) Serve(addr string) (err error) {
	...
	return s.ServePacketConn(pktConn)
}

func (s *Server) ServePacketConn(pktConn net.PacketConn) (err error) {
	quicMaxOpenIncomingStreams := int64(s.maxOpenIncomingStreams)
	s.packetConn = pktConn
...
func (s *Server) handleConn(conn quic.Connection) (err error) {
	...
	defer authDone()
	authenticatedUUID := make(chan string, 1)
	go func() {
		var (
		...
		if authUUID, uniStream, err = s.handleConnAuth(authCtx, conn); err != nil {
			s.logger.Warn().
				Err(err).
				Msg("handleAuth")
			cancel()
		...
		if authUUID != nil {
			authenticatedUUID <- authUUID.String()
		}
		authDone()
		for {
			select {
	...
	var userUUID string
	select {
	case userUUID = <-authenticatedUUID:
	case <-ctx.Done():
		return ctx.Err()
	}
	for {
		...
		go func(stream quic.Stream) {
			if err = s.handleStream(ctx, authCtx, conn, stream, userUUID); err != nil {
				s.logger.Warn().
					Err(err).
					Send()
			}
...
func (s *Server) handleStream(ctx context.Context, authCtx context.Context, conn quic.Connection, stream quic.Stream, userUUID string) error {
	...
	case "tcp":
		...
		defer rConn.Close()
		clientCounter := &trafficObservedConn{Conn: lConn, observer: s.observer, uuid: userUUID, clientIP: source, upload: true}
		remoteCounter := &trafficObservedConn{Conn: rConn, observer: s.observer, uuid: userUUID, clientIP: source}
		if err = s.relay.RelayTCP(clientCounter, remoteCounter); err != nil {
			var netErr net.Error
	...
	case "udp":
		...
		rConn := &trafficObservedPacketConn{PacketConn: c.(netproxy.PacketConn), observer: s.observer, uuid: userUUID, clientIP: source}
...
func (s *Server) Close() error {
	if s == nil || s.packetConn == nil {
		return nil
	}
	return s.packetConn.Close()
}

type trafficObservedConn struct {
	netproxy.Conn
	observer Observer
	uuid     string
	clientIP string
...
func (c *trafficObservedConn) Read(p []byte) (int, error) {
	n, err := c.Conn.Read(p)
	if n > 0 && c.observer != nil {
		if c.upload {
			c.observer.AddTraffic(c.uuid, int64(n), 0, c.clientIP)
		} else {
			c.observer.AddTraffic(c.uuid, 0, int64(n), c.clientIP)
		}
	}
	return n, err
}

type trafficObservedPacketConn struct {
	netproxy.PacketConn
	observer Observer
	uuid     string
	clientIP string
...
func (c *trafficObservedPacketConn) ReadFrom(p []byte) (int, netip.AddrPort, error) {
...
func (c *trafficObservedPacketConn) WriteTo(p []byte, addr string) (int, error) {
